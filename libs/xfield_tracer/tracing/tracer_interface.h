                     ...
                     bool DebugMsg=true)
{
    ...
    do{
        ...
        for(size_t i=0;i<UnsolvedPartitionIndex.size();i++)
        {
            ...
            try
            {
                traced=TraceSubPatch<TracerType>(currPartIndex,PTr,NewVertIdx,NewVertDir,
                                                 NewIsLoop,onlyneeded,only_non_disk,false,force_always);
            }
            catch (const std::exception& exc)
            {
                std::cerr<<"[WARN] TraceSubPatch failed for partition "
                         <<currPartIndex
                         <<" with "
                         <<PTr.Partitions[currPartIndex].size()
                         <<" faces: "
                         <<exc.what()
                         <<std::endl;
                traced=false;
